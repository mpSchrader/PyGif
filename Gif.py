...
class Gif (tk.Label):
	...
	def animate(self, threaded=True, interval=150, n_repeats=-1):
		...
		if threaded:
			animator = threading.Thread(target = self._play_gif, args = (interval, ))
			animator.start()
		else:
	...
	def _next_frame(self, interval):
		try:
			...
			self.frame = 0
			self.repeats += 1
			# In case the number of repeats has been set to a positive number
			# check if the number of repeats has exceded.
